...
class S3IterTransform:
    ...
    def __call__(self, s3_reader: S3Reader) -> Tuple[torch.Tensor, int]:
        content = s3_reader.read()
        image_pil = Image.open(BytesIO(content))
        image_tensor = self.transform(image_pil)
        label = int(s3_reader.key.split('/')[1])
...
def create_mnist_loader(bucket_name: str, split: str, loader_type:str, batch_size:int, num_workers: int=1,
                        smoke_test_count: int=0) -> Tuple[Any]:
    # Start of load time.
    ...
    if loader_type == 's3iter':
        uri = f's3://{bucket_name}/{split}'
        aws_region = os.environ['AWS_REGION']
        dataset = S3IterableDataset.from_prefix(uri, region=aws_region, enable_sharding=True,
                                                transform=S3IterTransform(transform))
        loader = DataLoader(dataset, batch_size=batch_size, num_workers=num_workers)
        return loader, (time.perf_counter()-start_time)

    if loader_type == 'itertar':
