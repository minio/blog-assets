#!/bin/bash
# load_iceberg_tables_sf5000.sh
# Loads all TPC-DS sf5000 tables into aistor.scale5000 via Starburst REST API.
...
SCHEMA="scale5000"
SOURCE="tpcds.sf5000"
LOG="/tmp/load_iceberg_tables_sf5000.log"

# All 25 TPC-DS tables ordered small → large
# Note: date_dim and catalog_page were missing from sf50000 script — included here
TABLES=(
    time_dim date_dim item customer customer_address customer_demographics
    household_demographics income_band promotion reason ship_mode
    warehouse call_center catalog_page store web_page web_site dbgen_version
...
run_query() {
    ...
    # Submit query
    local response
    response=$(curl -s -X POST "${COORDINATOR}/v1/statement"         -H "X-Trino-User: ${user}"         -H "X-Trino-Catalog: ${CATALOG}"         -H "Content-Type: text/plain"         --data "${sql}")
...
# Ensure schema exists before any CTAS
echo "Ensuring schema ${SCHEMA} exists..."
curl -s -X POST "${COORDINATOR}/v1/statement" \
    -H "X-Trino-User: admin" \
    -H "X-Trino-Catalog: ${CATALOG}" \
    -H "Content-Type: text/plain" \
    --data "CREATE SCHEMA IF NOT EXISTS ${SCHEMA}" > /dev/null
sleep 3

echo "=== TPC-DS sf5000 → aistor.scale5000 ===" | tee -a "$LOG"
...
for table in "${TABLES[@]}"; do
    ...
    check=$(curl -s -X POST "${COORDINATOR}/v1/statement"         -H "X-Trino-User: admin"         -H "Content-Type: text/plain"         --data "SELECT count(*) FROM ${CATALOG}.${SCHEMA}.${table} LIMIT 1")
