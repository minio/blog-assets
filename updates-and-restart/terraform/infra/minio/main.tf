...
module "hello_minio_instance_minio" {
  ...
  for_each = var.hello_minio_public_igw_cidr_blocks

  minio_vpc_security_group_ids = [module.hello_minio_security_group_minio.minio_security_group_id]
  minio_subnet_id              = module.hello_minio_vpc.minio_subnet_public_igw_map[each.key]
...
locals {
      ...
      minio_instances_public_ips = [
        for each_instance in module.hello_minio_instance_minio :
        each_instance.minio_instance_public_ip
      ]

      minio_hostname_public_ip_map = {
        for i, ip in local.minio_instances_public_ips :
          "server-${i+1}.minio.local" => ip
    }

    minio_instances_private_ips = [
        for each_instance in module.hello_minio_instance_minio :
        each_instance.minio_instance_private_ip
      ]

      minio_hostname_private_ip_map = {
        for i, ip in local.minio_instances_private_ips :
          "server-${i+1}.minio.local" => ip
    }
