...
def train_model(model: nn.Module, loader: DataLoader, training_parameters: Dict[str, Any]) -> List[float]:
    ...
    for epoch in range(training_parameters['epochs']):
        ...
        logger.info(f'Epoch {epoch+1} - Samples: {sample_count} - Training loss: {total_loss/batch_count:.4f} - Compute time: {epoch_compute_time:.4f} IO time: {epoch_io_time:.4f}.')
...
def setup_local_training(training_parameters: Dict[str, Any], loader_type: str):
    ...
    train_loader, load_time_sec = ml.create_mnist_loader(training_parameters['bucket_name'], 'train', loader_type, training_parameters['batch_size'], 
    ...
    # Create the model.
    model = mm.MNISTModel(training_parameters['input_size'], training_parameters['hidden_sizes'], training_parameters['output_size'])
...
def setup_local_testing(training_parameters: Dict[str, Any], loader_type: str):
    ...
    model.eval()
    test_loader, load_time_sec = ml.create_mnist_loader(training_parameters['bucket_name'], 'test', loader_type, training_parameters['batch_size'])
...
def main():
    ...
    parser.add_argument('-lt', '--loader_type', help='Type of loader to use for loading training and test sets (file, list, memory, map, iter, itertar, s3map or s3iter).')
